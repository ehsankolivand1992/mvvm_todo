...
class EditTaskDialog(taskModel: TaskModel,context: Activity, fragToDo: FragToDo,fragDone: FragDone,page: Int): View.OnClickListener{
   ...
   lateinit var ll_first: LinearLayout
    ...
    init {
        ...
        this.labelValue = taskModel.value
        when(page)
        {
            1->{
                this.listinner =fragToDo
            }
            2->{
                this.listinner =fragDone
    ...
    fun ShowTaskDialog()
    {
        ...
        setDefultLable(taskModel.value)

        ed_tite.setText(taskModel.title)
        ed_description.setText( taskModel.description)
    ...
    private fun getData(title: String, desctipion: String) {
        if (title.isNotEmpty())
        {
            if (desctipion.isNotEmpty())
            {
                ...
                taskModel.description = desctipion
                taskModel.value = labelValue

                listinner.addTaskToDatabase(taskModel)
